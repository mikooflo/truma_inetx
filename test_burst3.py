#!/usr/bin/env python3
"""
Test: rafale TRÈS serrée comme l'iNetX
"""
import serial, time, sys, fcntl, termios, select

UART = '/dev/serial0'
ser = serial.Serial(UART, 9600, timeout=0.020)
time.sleep(0.2)
...
try:
    ...
    while True:
        ...
        t1 = time.time()
        bt = t1 - t0
        print(f"  burst={bt*1000:.0f}ms  sleep={13.0-bt:.1f}s")
        if bt < 13.0:
            time.sleep(13.0 - bt)
